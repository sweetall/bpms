...
def write_login_log(username, type='', ip='', user_agent=''):
    ...
    LoginLog.objects.create(
        ...
        ip=ip, user_agent=user_agent  # city=city,
    )
...
def generate_otp_uri(request, issuer="bpms"):
...
def get_user_info(request):
    cache_key = 'USER_INFO_%s' % request.user.username.upper()
    user_info = cache.get(cache_key)
    if not user_info and settings.LOPS_AUTH:
        try:
            r = requests.get(settings.LOPS_USER_INFO_API % request.user.username)
            r = r.json()
            if r.get('status'):
                user_info = r.get('data')
                cache.set(cache_key, user_info, 60*60*12)
                return user_info
        except Exception:
            return None
    return user_info
