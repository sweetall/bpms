...
INSTALLED_APPS = [
    ...
    'transfer.apps.TransferConfig',  # add
    'rest_framework',
...
MIDDLEWARE = [
    ...
    'bpms.middleware.TimezoneMiddleware',
    'bpms.middleware.DemoMiddleware',
]
...
TEMPLATES = [
    {
        ...
        'DIRS': [
            ...
            os.path.join(BASE_DIR, 'perms/templates'),
            os.path.join(BASE_DIR, 'terminal/templates'),
            os.path.join(BASE_DIR, 'users/templates'),
        ],
        'APP_DIRS': True,
        'OPTIONS': {
            'context_processors': [
                'bpms.context_processor.bpms_processor',
...
# WSGI_APPLICATION = 'bpms.wsgi.applications'
...
LOGGING = {
    ...
    'handlers': {
        ...
        'file': {
            ...
            'formatter': 'main',
            'filename': os.path.join(PROJECT_DIR, 'logs', 'bpms.log')
        },
        'ansible_logs': {
            'level': 'DEBUG',
            'class': 'logging.FileHandler',
...
CELERY_BROKER_URL = 'redis://:%(password)s@%(host)s:%(port)s/2' % {
...
LOPS_AUTH = CONFIG.LOPS_AUTH

if LOPS_AUTH:
    LOGIN_URL = '/login/'
    LOGIN_REDIRECT_URL = '/bpms/'
    AUTHENTICATION_BACKENDS = ['common.lops_api.backends.APIAuthBackend', ]
    EMAIL_BACKEND = 'common.lops_api.backends.EmailBackend'
    LOPS_HOST = CONFIG.LOPS_HOST
    LOPS_AUTH_API = os.path.join(LOPS_HOST, 'core/api/auth/token/')
    LOPS_EMAIL_API = os.path.join(LOPS_HOST, 'core/api/email/')
    LOPS_SENDER_NAME = u'寿险业务量预测系统'
    LOPS_USER_INFO_API = os.path.join(LOPS_HOST, 'core/api/user/%s/')
