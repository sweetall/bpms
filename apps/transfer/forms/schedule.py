...
class ImportScheduleCreateForm(forms.Form):
    ...
    def save(self, request):
        ...
        task_info = {
            ...
            'command_list': create_transfer_cmd(database, tables_id_list),
            'user': request.user
        }
...
class ImportScheduleUpdateForm(forms.Form):
    ...
    def clean_schedule(self):
        return get_object_or_none(TransferSchedule, id=self.cleaned_data['schedule'])
...
class ExportScheduleCreateForm(forms.Form):
    from_schedule = forms.ModelChoiceField(queryset=TransferSchedule.objects.filter(type=0, status=4),
                                           required=True, label='选择任务 *')
    tables = forms.MultipleChoiceField(
        required=True,
        label='选择表 *', choices=list(set(Command.objects.filter(status=2)
    ...
    def save(self, request):
        ...
        task_info = {
            ...
            'schedule_defaults': {
                ...
                'run_time': self.cleaned_data.get('run_time'),
                'type': 1,
                'from_schedule': str(from_schedule.id),
                'comment': self.cleaned_data.get('comment'),
            },
            'command_list': create_transfer_cmd(database, tables_id_list),
            'user': request.user
...
class ExportScheduleUpdateForm(forms.Form):
    ...
    def clean_schedule(self):
        return get_object_or_none(TransferSchedule, id=self.cleaned_data.get('schedule'))
