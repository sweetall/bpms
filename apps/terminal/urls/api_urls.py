...
urlpatterns = [
    re_path(r'^v1/sessions/(?P<pk>[0-9a-zA-Z\-]{36})/replay/$',
            api.SessionReplayViewSet.as_view({'get': 'retrieve', 'post': 'create'}),
            name='session-replay'),
    re_path(r'^v1/tasks/kill-session/', api.KillSessionAPI.as_view(), name='kill-session'),
    re_path(r'^v1/terminal/(?P<terminal>[a-zA-Z0-9\-]{36})/access-key', api.TerminalTokenApi.as_view(),
            name='terminal-access-key'),
    re_path(r'^v1/terminal/config', api.TerminalConfig.as_view(), name='terminal-config'),
    # v2: get session's replay
    re_path(r'^v2/sessions/(?P<pk>[0-9a-zA-Z\-]{36})/replay/$',
